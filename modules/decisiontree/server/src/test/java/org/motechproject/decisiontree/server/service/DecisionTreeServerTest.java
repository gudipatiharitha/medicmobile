...
public class DecisionTreeServerTest {
    ...
    public void shouldThrowExceptionForInvalidTransition() {
        ...
        Assert.assertEquals(DecisionTreeServer.Error.INVALID_TRANSITION_KEY.toString(), modelAndView.getModel().get("message"));
    }

    @Test
    public void shouldReturnErrorForInvalidTree() {
        flowSession.set(DecisionTreeServer.CURRENT_NODE_PARAM, null);

        ModelAndView modelAndView = decisionTreeServer.getResponse(flowSession.getSessionId(), "1234567890", "freeivr", null, "1", "en");

        assertEquals("/vm/error-freeivr", modelAndView.getViewName());
        assertEquals(DecisionTreeServer.Error.TREE_OR_LANGUAGE_MISSING.toString(), modelAndView.getModel().get("message"));
    }
