...
import org.motechproject.appointments.api.model.search.OriginalDueDateInCriteria;
import org.motechproject.appointments.api.model.search.UnvisitedCriterion;
import org.motechproject.appointments.api.model.search.VisitDateInCriterion;
...
public class VisitsQuery {
    ...
    public VisitsQuery withVisitDateIn(DateTime start, DateTime end) {
        criteria.add(new VisitDateInCriterion(start, end));
        return this;
    }
    /**
     * Adds query to filter visits with original due date in the given range. (Start and end time inclusive)
     *
     * @param start
     * @param end
     * @return
     */   
    public VisitsQuery withOriginalDueDateIn(DateTime start, DateTime end) {
        criteria.add(new OriginalDueDateInCriteria(start, end));
        return this;
    }
